              ...
              {searchResult && (
                <div
                  style={{
                    ...
                    background: "#0f172a",
                    border: `1px solid ${
                      getToxicityVerdict(searchResult.mean).color
                    }`,
                    color: "#e5e7eb",
                    fontSize: 13,
                  }}
                >
                  <div
                    style={{
                      marginBottom: 6,
                      fontWeight: 700,
                      color: getToxicityVerdict(searchResult.mean).color,
                      display: "flex",
                      alignItems: "center",
                      gap: 6,
                    }}
                  >
                    <span>{searchResult.verdictEmoji}</span>
                    <span>
                      Thread: {searchResult.thread_id} •{" "}
                      {searchResult.verdictLabel}
                    </span>
                  </div>
                  <div style={{ marginBottom: 2 }}>💬 Comments: {searchResult.count}</div>
                  <div style={{ marginBottom: 2 }}>
                    📊 Mean: {(searchResult.mean * 100).toFixed(1)}%
                  </div>
                  <div style={{ marginBottom: 2 }}>
                    📈 P90: {(searchResult.p90 * 100).toFixed(1)}%
                  </div>
                  <div style={{ marginBottom: 2 }}>
                    📉 P95: {(searchResult.p95 * 100).toFixed(1)}%
                  </div>
                  <div>
                    ⚠️ Max: {(searchResult.max * 100).toFixed(1)}%
                  </div>
                </div>
              )}
