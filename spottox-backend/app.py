...
# -----------------------------------------------------------
# Reddit Parsing Helpers (FULL VERSION WITH RECURSION + MORE)
# -----------------------------------------------------------

def extract_all_comments(children, more_ids):
    """
    Recursively extracts all comment bodies from Reddit JSON.
    Collects 'more' comment IDs for later expansion.
    """
    comments = []
...
def fetch_more_comments(thread_id, more_ids):
    """
    Fetch missing comments from Reddit's /api/morechildren endpoint.
    Handles collapsed comments.
    """
    if not more_ids:
        return []
...
def analyze_reddit():
    ...
    # Extract full comment tree + collect "more" IDs
    try:
    ...
    # Fetch missing collapsed comments
